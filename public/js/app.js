...
class App {
  ...
  async init() {
    ...
    // Bind Backend configuration controls (Render / Vercel serverless / Local)
    this.setupBackendControls();
  }

  setupBackendControls() {
    const btnBackend = document.getElementById("btn-backend-config");
    const label = document.getElementById("backend-status-label");

    const updateLabel = () => {
      const current = window.getBackendUrl ? window.getBackendUrl() : "";
      if (label) {
        if (current) {
          try {
            const host = new URL(current).hostname;
            label.textContent = `API: ${host}`;
          } catch (e) {
            label.textContent = "API: Custom";
          }
        } else {
          label.textContent = "API: Auto";
        }
      }
    };

    updateLabel();
    window.addEventListener("backendUrlChanged", updateLabel);

    btnBackend?.addEventListener("click", () => {
      const current = window.getBackendUrl ? window.getBackendUrl() : "";
      const input = prompt(
        "KalaSetu AI - Backend Connection Setup:\n\n" +
        "Enter your backend URL (e.g., https://your-service.onrender.com),\n" +
        "or leave blank to use the default same-origin / Vercel serverless backend:",
        current || ""
      );
      if (input !== null) {
        if (window.setBackendUrl) {
          window.setBackendUrl(input);
        }
        if (typeof showToast === "function") {
          showToast(input.trim() ? `Connected to backend: ${input.trim()}` : "Backend reset to default (same-origin)", "info");
        }
      }
    });
  }
