...
// Dynamic API Base resolver (supports same-origin, custom Render URL, and query override)
function getResolvedApiBase() {
  if (typeof window !== "undefined") {
    try {
      const urlParams = new URLSearchParams(window.location.search);
      const queryBackend = urlParams.get("backend_url");
      if (queryBackend) {
        const clean = queryBackend.trim().replace(/\/+$/, "");
        localStorage.setItem("kalasetu_backend_url", clean);
        return clean;
      }
    } catch (e) {}

    const saved = localStorage.getItem("kalasetu_backend_url");
    if (saved && saved.trim()) {
...
window.setBackendUrl = function(url) {
  if (!url || !url.trim()) {
    localStorage.removeItem("kalasetu_backend_url");
    API_BASE = "";
    console.log("[KalaSetu] Backend URL reset to default same-origin.");
  } else {
    const clean = url.trim().replace(/\/+$/, "");
    localStorage.setItem("kalasetu_backend_url", clean);
    API_BASE = clean;
    console.log("[KalaSetu] Backend URL configured to:", clean);
  }
  window.dispatchEvent(new CustomEvent("backendUrlChanged", { detail: { backendUrl: API_BASE } }));
};
