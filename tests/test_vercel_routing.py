...
class TestVercelRouting(unittest.TestCase):
    ...
    def test_upload_and_enhance_via_vercel_now_route_matches(self):
        # Test POST upload-and-enhance rewritten to /api/index.py with x-now-route-matches (Vercel standard)
        from PIL import Image
        img_byte_arr = io.BytesIO()
        img = Image.new("RGB", (100, 100), color=(180, 100, 60))
        img.save(img_byte_arr, format="JPEG")
        ...
        res = self.client.post(
            ...
            headers={
                **self.auth_headers,
                "x-now-route-matches": "1=ai%2Fupload-and-enhance"
            },
            files=files,
            data=data
        )
